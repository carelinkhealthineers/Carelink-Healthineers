...
export const VideoTheatreModal: React.FC<VideoTheatreModalProps> = ({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[999] flex flex-col items-center justify-center bg-slate-950/95 backdrop-blur-2xl p-2 sm:p-4 md:p-6 selection:bg-blue-600 selection:text-white overflow-hidden">
        ...
        <motion.div 
          ...
          className={`relative w-full ${isTheatreExpanded ? 'max-w-[1500px]' : 'max-w-5xl'} max-h-[92vh] sm:max-h-[94vh] bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col transition-all duration-300 z-10`}
        >
          {/* Top Bar / Controls */}
          <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 bg-slate-950/95 border-b border-slate-800 text-white z-10 shrink-0">
          ...
          <div className="relative bg-black w-full flex-1 min-h-0 overflow-hidden flex items-center justify-center">
            <div className="w-full h-full max-h-[55vh] sm:max-h-[62vh] aspect-video relative flex items-center justify-center">
              {parsed.type === 'youtube' || parsed.type === 'vimeo' || parsed.type === 'iframe' ? (
                ...
                <video 
                  ...
                  autoPlay
                  playsInline
                  className="w-full h-full object-contain max-h-[55vh] sm:max-h-[62vh] bg-black"
                />
              )}
            </div>
            ...
            {playlist.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-3 top-1/2 -translate-y-1/2 p-2.5 sm:p-3 rounded-2xl bg-slate-900/80 hover:bg-blue-600 text-white border border-slate-700/60 disabled:opacity-20 disabled:pointer-events-none transition-all shadow-xl backdrop-blur-md"
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-2.5 sm:p-3 rounded-2xl bg-slate-900/80 hover:bg-blue-600 text-white border border-slate-700/60 disabled:opacity-20 disabled:pointer-events-none transition-all shadow-xl backdrop-blur-md"
                  title="Next Video"
                >
